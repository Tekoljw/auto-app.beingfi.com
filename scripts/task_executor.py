...
from model import parse_explore_rsp, parse_grid_rsp, OpenAIModel, QwenModel, DeepSeekModel
...
if device_mode == "vmos":
    ...
    # 处理api_host，提取主机名用于签名（VMOSCloudController内部会处理完整URL）
...
while round_count < configs["MAX_ROUNDS"]:
    ...
    if not os.path.exists(screenshot_path):
        print_with_color(f"ERROR: Screenshot file does not exist: {screenshot_path}", "red")
        break
    if grid_on:
        try:
        ...
        image = os.path.join(task_dir, f"{dir_name}_{round_count}_grid.png")
            ...
            print_with_color(f"ERROR: Failed to draw grid: {e}", "red")
            break
