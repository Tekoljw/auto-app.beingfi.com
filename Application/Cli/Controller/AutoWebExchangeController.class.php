 ...
 */
class AutoWebExchangeController extends ExchangeBaseController {
    ...
    public function __construct() {
        ...
        $this->channelDocDir = $root . '/Public/WebAutoScriptDoc';
    ...
    protected function _submitReceiptsForAccount($account, $receipts) {
        return ExchangeCommon::submitReceiptsForAccount($this->runtimePath, $account, $receipts);
    }
    ...
    protected function _runNodeScript($action, $step, $doc, $account) {
        ...
        if ($action === 'executePayment' && !empty($orders) && !empty($decoded['order_results'])) {
            ...
            ExchangeCommon::processPaymentOrderResults($this->runtimePath, $decoded['order_results'], $orders, $account, isset($payLog) ? $payLog : null);
        }
